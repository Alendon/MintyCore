...
namespace Veldrid
{
	...
	public abstract class CommandList : DeviceResource, IDisposable
	{
		...
		public CommandList GetSecondaryCommandList()
		{
			if ( IsSecondary ) throw new VeldridException( "A secondary command list can only be aquired from a primary command list" );
			CommandList cl = GetSecondaryCommandListCore();
			cl._isSecondary = true;
			return cl;
		}
