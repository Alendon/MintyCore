...
class Build : NukeBuild
{
    ...
    List<string> GetModDependencyPackageNames(Project project)
    {
        ...
        return modInfoNode.SelectNodes("Dependency").Cast<XmlNode>().Select(x => x.Attributes["PackageName"].Value)
            .ToList();
    }
    ...
    void ValidateManifest(ModManifest manifest)
    {
        ...
        Assert.True(Version.TryParse(manifest.Version, out _),
            "Mod Version is not in a valid format. It should be in the format of \"Major.Minor.Patch\"");
    ...
    ModManifest ReadManifest(Project project)
    {
        ...
        manifest.Version = baseNode?.SelectSingleNode("Version")?.InnerText ?? String.Empty;
...
class ModManifest
{
    ...
    public List<Dependency> ExternalDependencies { get; set; } = new();
}