    ...
    class ParallelSystemGenerator : ISourceGenerator
    {
        ...
        public void Execute(GeneratorExecutionContext context)
        {
            ...
            foreach (var parallelClass in parallelClasses)
            {
                ...
                compilationUnit =
                    compilationUnit.WithUsings((parallelClass.Parent.Parent as CompilationUnitSyntax).Usings);
                compilationUnit =
        ...
        private ClassDeclarationSyntax GenerateParallelExtensionClass(ClassDeclarationSyntax parallelClass)
        {
            ...
            var queryField =
                parallelClass.Members.Where(member =>
                        member.AttributeLists.Any(attribute => attribute.ToString().StartsWith("[ComponentQuery")))
                    .First() as FieldDeclarationSyntax;
...
";
        ...
        private static void SearchParallelSystems(GeneratorExecutionContext context,
            List<ClassDeclarationSyntax> parallelClasses, HashSet<ClassDeclarationSyntax> classesToCheck,
            string attributeName)
        {
            foreach (var classDeclaration in from syntaxTree in context.Compilation.SyntaxTrees
                     ...
                     where classDeclaration.BaseList != null &&
                           classDeclaration.AttributeLists.Any(x => x.ToString().StartsWith($"[{attributeName}"))
