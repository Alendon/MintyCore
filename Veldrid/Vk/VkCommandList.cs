...
namespace MintyVeldrid.Vk
{
	internal unsafe class VkCommandList : CommandList
	{
		...
		private readonly object _secondaryCommandBufferListLock = new object();
		private readonly Queue<VkCommandBuffer> _availableSecondaryCommandBuffers = new Queue<VkCommandBuffer>();
		...
		public VkCommandList(VkGraphicsDevice gd, ref CommandListDescription description, VkCommandList parent, VkCommandBuffer commandBuffer, VkCommandPool pool)
			: base(ref description, gd.Features, gd.UniformBufferMinOffsetAlignment, gd.StructuredBufferMinOffsetAlignment)
		{
			_gd = gd;
			_parent = parent;
		...
		public void CommandBufferCompleted(VkCommandBuffer completedCB)
		{
			...
			{
				HashSet<VkCommandList> associatedSecondaryCLs = _primaryUsedSecondaryCommandLists.ContainsKey(completedCB) ? _primaryUsedSecondaryCommandLists[completedCB] : new HashSet<VkCommandList>(0);
				foreach (var secondaryList in associatedSecondaryCLs)
				{
					_secondaryCommandListUseCount[secondaryList]--;
					if (_secondaryCommandListUseCount[secondaryList] != 0 || !_returnedSecondaryCommandLists.Contains(secondaryList))
					{
						continue;
					}
					_secondaryCommandListUseCount.Remove(secondaryList);
					_availableSecondaryCommandBuffers.Enqueue(secondaryList.CommandBuffer);
					_returnedSecondaryCommandLists.Remove(secondaryList);
				}
				if (_primaryUsedSecondaryCommandLists.ContainsKey(completedCB)) _primaryUsedSecondaryCommandLists[completedCB].Clear();
			}
		...
		private protected override void SetPipelineCore(Pipeline pipeline)
		{
			...
			Stopwatch nullWatch = Stopwatch.StartNew();

			while (_currentStagingInfo == null)
			{
			}
			nullWatch.Stop();
			if (nullWatch.Elapsed.Ticks > 4)
			{
		...
		public override void ExecuteSecondaryCommandList(CommandList commandList)
		{
			lock (this)
			{
				if (IsSecondary)
				{
					throw new VeldridException("A secondary command list can only executed from a primary command list");
				}
				if (!commandList.IsSecondary)
				{
					throw new VeldridException("Only a secondary command list can be executed by a primary command list");
				}
				if (commandList is not VkCommandList)
				{
					throw new VeldridException("Invalid command list type");
				}
				VkCommandList vkCommandList = commandList as VkCommandList;
				if (vkCommandList._pool != _pool)
				{
				...
				lock (_secondaryCommandBufferListLock)
				{
					...
					{
						_primaryUsedSecondaryCommandLists.Add(CommandBuffer, new HashSet<VkCommandList>());
					}
					_primaryUsedSecondaryCommandLists[CommandBuffer].Add(vkCommandList);

					if (!_secondaryCommandListUseCount.ContainsKey(vkCommandList))
					{
						_secondaryCommandListUseCount.Add(vkCommandList, 0);
					}
					_secondaryCommandListUseCount[vkCommandList]++;
				}

				vkCmdExecuteCommands(CommandBuffer, 1, ref vkCommandList._cb);
			}
		}
		...
		{
			vkCmdPushConstants(_cb, ((VkPipeline)_graphicsPipeline).PipelineLayout, VkFormats.VdToVkShaderStages(pushConstant.ShaderStages), pushConstant.Offset, pushConstant.Size, &pushConstant.Value);
		}

		protected override void FreeSecondaryCommandListCore()
		{
			if (!IsSecondary)
			{
		...
		private void FreeSecondaryCommandList(VkCommandList list)
		{
			lock (_secondaryCommandBufferListLock)
			{
				_returnedSecondaryCommandLists.Add(list);
			}
		}

		private VkCommandBuffer GetNextSecondaryCommandBuffer()
		{
			lock (_secondaryCommandBufferListLock)
			{
				if (_availableSecondaryCommandBuffers.Count > 0)
				{
					VkCommandBuffer cachedCB = _availableSecondaryCommandBuffers.Dequeue();
					VkResult resetResult = vkResetCommandBuffer(cachedCB, VkCommandBufferResetFlags.None);
					CheckResult(resetResult);
					return cachedCB;
				}
