...
namespace MintyCore.Render
{
    ...
    public static class PipelineHandler
    {
        ...
        {
            DescriptorSetLayout* pDescriptorSets = stackalloc DescriptorSetLayout[description.descriptorSets.Length];

            for (int i = 0; i < description.descriptorSets.Length; i++)
            {
            ...
            PipelineShaderStageCreateInfo* shaderInfos =
                stackalloc PipelineShaderStageCreateInfo[description.shaders.Length];
            for (int i = 0; i < description.shaders.Length; i++)
            {
                Shader shader = ShaderHandler.GetShader(description.shaders[i]);
