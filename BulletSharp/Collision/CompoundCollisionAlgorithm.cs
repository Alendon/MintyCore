...
namespace MintyBulletSharp
{
	public class CompoundCollisionAlgorithm : ActivatingCollisionAlgorithm
	{
		...
		{
			public CreateFunc(ConstructionInfo info)
				: base(info)
			{
			}
		...
		{
			public SwappedCreateFunc(IntPtr native, BulletObject owner)
				: base(ConstructionInfo.Null)
			{
				InitializeSubObject(native, owner);
			}
		...
		protected CompoundCollisionAlgorithm()
		{
		}

		public CompoundCollisionAlgorithm(IntPtr native, BulletObject owner)
		{
			InitializeSubObject(native, owner);
		}
