...
/// </summary>
public static class AsyncFenceAwaiter
{
    private static Thread? _thread;
    private static volatile bool _running;

    private static readonly Dictionary<ManagedFence, (TaskCompletionSource<bool> tcs, DateTime timeout,
        CancellationToken cancellationToken)> _awaiters =
        new();

    private static readonly ManualResetEvent _newAwaiterMutex = new(false);
    ...
    [PublicAPI]
    public static Task<bool> AwaitAsync(this ManagedFence fence, uint timeout = uint.MaxValue,
    ...
    private static void MarkFencesCompleted()
    {
        lock (_awaiters)
        {
    ...
    private static void GetActiveFences(ref Span<Fence> fences, out int fenceCount)
    {
        fenceCount = 0;
        lock (_awaiters)
        {
            ...
            foreach (var managedFence in _awaiters.Keys.ToArray())
            {
                ...
                fences[fenceCount++] = managedFence.InternalFence;
            }
        }
    }
